...
function initSlots() {
  ...
  for (var i = 0; i < slots.length; i++) {
    slots[i].addEventListener('click', (function(i) {
      var onclick = function onclick() { toggleAddToSelection(i); };
      var slot = slots[i];
...
function initHoverListn() {
  ...
  for (var i = 0; i < slots.length; i++) {
    slots[i].addEventListener('mouseenter', (function(i) {
      var onhover = function() { setCursor(i); };
      var slot = slots[i];
...
//
// entry :: Number, scroll :: Boolean
function setCursor (entry, scroll) {
  if (slots.length === 0) return;
  entry %= slots.length;
  if (entry < 0) { entry = slots.length - 1; }
  if (pointer >= 0) { slots[pointer].classList.remove('focus'); }
  pointer = entry;
  var slot = slots[pointer];
  slot.classList.add('focus');
  if (scroll) {
    var box = slot.getBoundingClientRect();
    var viewportHeight = window.innerHeight;
    // We remove 30px for the top toolbar.
    var slotInvisible = (box.top - 30 < 0 || box.bottom > viewportHeight);
    if (slotInvisible) {
      filelist.scrollTop += box.top - ((viewportHeight / 2) >>> 0);
      setTimeout(function() { setCursor(entry); }, 100);
    }
  }
}

function nextEntry () { setCursor(pointer + 1, true); }

function prevEntry () { setCursor(pointer - 1, true); }
