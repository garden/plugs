...
function initSlots() {
  ...
  for (var i = 0; i < slots.length; i++) {
    slots[i].addEventListener('click',
                             (function(i) { return function onclick() {
      setCursor(i);
      hideMenu();
    }; }(i)));
  }
...
var menuBut = document.getElementById('filemenubut');
var menuUIToggle = false;
function initializeMenus() {
  menuBut.addEventListener('click', function clickedMenuBut() {
    if (menuUIToggle) { hideMenu(); } else {
    var selectedSlot = slots[pointer];
    menuBut.style.background = 'radial-gradient(circle at bottom,white 15%,rgba(255,255,255,0) 75%)';
    menuBut.style.opacity = '1';
    // Show the menu UI at the correct spot.
    var menuButBounds = menuBut.getBoundingClientRect();
    menuUI.style.top = menuButBounds.bottom + 'px';
    menuUI.style.left = menuButBounds.left + 'px';
    // Put stuff in there.
    menuUI.innerHTML = '';
    ...
    deleteBut.onclick = function() {
      Scout.send(function(query, xhr) {
        ...
        query.data = {
          op: 'rm',
          path: selectedSlot.firstElementChild.getAttribute('href'),
        };
        query.resp = function(data) {
          if (data.err != null) {
...
function hideMenu() {
  ...
  menuBut.style.background = 'transparent';
  menuUIToggle = false;
}
