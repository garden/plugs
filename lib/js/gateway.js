...
function onDeleteFile(event) {
  if (confirm('Do you really wish to delete those '
              + nSelectedFiles + ' files?')) {
    Object.keys(selectedFile).forEach(function (selectedFileIndex) {
      ...
      Scout.send(function(query, xhr) {
        ...
        query.resp = function(data) {
          if (data.err != null) {
            alert('The files did not get deleted for some unspecified reason.'
