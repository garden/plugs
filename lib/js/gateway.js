...
var fileSelectionEvenListenerReset = [];
...
function initSlots() {
  ...
  // File selection.
  for (var i = 0; i < fileSelectionEvenListenerReset.length; i++) {
    fileSelectionEvenListenerReset[i]();
  }
  fileSelectionEvenListenerReset = [];
  for (var i = 0; i < slots.length; i++) {
    slots[i].addEventListener('click',
                             (function(i) {
      var onclick = function onclick() { toggleAddToSelection(i); };
      var slot = slots[i];
      fileSelectionEvenListenerReset.push(function resetOnClick() {
...
var deleteFileBut = document.getElementById('deletefilebut');
