...
Scout.eventSource('dispatch').onrecv(function (resp) {
  console.log ('plug\t, plugger\t,',client.path,'\t, received', resp);
...
function sending (delta) {
  return function fsending (params) {
    ...
    // DEBUG
    console.log('plug\t, plugger\t,',client.path,'\t, sending: ' + JSON.stringify(params.data));
    params.resp = function () {
      console.log ('plug\t, plugger\t,',client.path,'\t, sent');
      acked = true;
      if (resend) {
        console.log('plug\t, plugger\t,',client.path,'\t, resending now!');
        resend = false;
    ...
    params.error = function senderror (status) {
      console.log('plug\t, plugger\t,',client.path,'\t, send error: status', JSON.stringify(status));
    };
...
function getdata() {
  Scout.send (function (params) {
    ...
    params.resp = function (resp) {
      console.log ('plug\t, plugger\t,',client.path,'\t, got content');///
