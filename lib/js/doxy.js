...
function parseComment( comment )
{
    ...
    // Regex to delete the first star, or / / !, if present, and the first spaces.
...
function doxygen( data ) {
    ...
    {
        if( !comment_mode )
        // We're looking for a "/ * *" mark or a "/ / !" mark
        {
            ...
            // We're looking for the termination mark "* /" or a new line without any / / !
            // Looking for */ mark
            if( javadoc_style_comment
                && pos+1 < total_length
                && str[pos] == '*' && str[pos+1] == '/'
            ...
            if( !comment_mode )
            {
                ...
                var rest = str.substring( com_end );
                var nextLine = rest.split('\n')[1];
                var objectId = infos.id || guessKey( nextLine );
