...
typejax.updater = {
  ...
  gettask : function() {
    ...
    if (typejax.totalsize == instext.length) {
      ...
      console.log('plug\t, typejax\t,',"innerdata:", typejax.innerdata);
      ...
      console.log('plug\t, typejax\t,',"div:",divstart,divend,"modify:",modstart,modend + modsize);
...
typejax.parser = function(input, modstart, modend){
    ...
    newEnding : function() {
      for (var i = 0; i < typejax.totaldata.length; i++) {
        if (typejax.totaldata[i][0] == this.modend) {
          ...
          console.log('plug\t, typejax\t,',"newEnding:",this.modend);
          break;
        }
      }
    },
  ...
  var syner = {
    ...
    doThisGroup : function() {
      ...
      if (this.nodelevel == 0) {
        if (!node.to) console.log('plug\t, typejax\t,',"doThisGroup: node.to is empty!");
        if (node.to > node.from) {
    ...
    openChild : function(type, name, from, mark) {
      ...
      if (!parent) {
        console.log('plug\t, typejax\t,',"openChild: wrong nodeplace!");
        return;
      }
      console.log('plug\t, typejax\t,',"OpenChild: ", type, name, from);
      var node = {
        type: type,
        name: name,
      ...
      console.log('plug\t, typejax\t,',"nodelevel:", this.nodelevel, "arglength:", node.argarray.length);
    ...
    closeChild : function(position) {
      ...
      node.to = position;
      if (!node) {
        console.log('plug\t, typejax\t,',"closeChild: wrong nodeplace!");
        return;
      }
      console.log('plug\t, typejax\t,',"CloseChild:", node.type, node.name, node.to);
      ...
      console.log('plug\t, typejax\t,',"nodelevel:", this.nodelevel);
      //this.printTree(this.innertree);
    },
    ...
    printTree : function(tree, spaces) {
      if (!spaces) spaces = "";
      console.log('plug\t, typejax\t,',"|" + spaces + tree.mode, tree.name, tree.from, tree.to, tree.value);
    ...
    appendValue : function(node, value, position) {
      if (!node) {
        console.log('plug\t, typejax\t,',"appendValue: wrong node!");
        return;
      }
    ...
    addText : function(value, position) {
      //if (arguments.length == 1) console.log('plug\t, typejax\t,',"no position for " + value);
      ...
      if (this.nodelevel > 0) {
        ...
        if (node.argarray.length == node.argtype.length) {
          ...
          if (node.argarray.length == node.argtype.length) {
            ...
            console.log('plug\t, typejax\t,',"addText: value is not empty!");
            this.addText(value, position + n - value.length);
          }
        }
      } else {
        console.log('plug\t, typejax\t,',"addText: nodelevel is zero!");
    ...
    includeGroup : function(name1, name2) {
      ...
      if ((mode1 == "block" && mode2 != "inline") || (mode1 == "inline")) {
        console.log('plug\t, typejax\t,',"includeGroup:", name1, name2, false);
        return false;
      }
  ...
  console.log('plug\t, typejax\t,',"outhtml:", outhtml);
  return outhtml;
};
